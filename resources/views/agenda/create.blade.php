    ...
    <form action="{{ route('admin.agenda.store') }}" method="POST" class="needs-validation" novalidate>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="judul" name="judul" value="{{ old('judul') }}"
                placeholder="Contoh: Rapat Kerja Bakti Bulanan" required>
        </div>
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" id="lokasi" name="lokasi" value="{{ old('lokasi') }}"
                    placeholder="Contoh: Balai RW, Lapangan Voli">
            </div>
        </div>
        <div class="mb-3">
            <label for="deskripsi" class="form-label">Deskripsi</label>
            <textarea class="form-control" id="deskripsi" name="deskripsi" rows="5"
                placeholder="Jelaskan detail agenda di sini...">{{ old('deskripsi') }}</textarea>
        </div>
        <hr>
